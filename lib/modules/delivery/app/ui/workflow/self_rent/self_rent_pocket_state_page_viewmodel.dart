...
import 'package:santapocket/modules/delivery/app/ui/workflow/result/success/self_rent_pocket_complete_page.dart';
...
@injectable
class SelfRentPocketStatePageViewModel extends AppViewModel {
  ...
  SelfRentPocketStatePageViewModel(
    ...
    await run(
      () async {
        if (delivery?.pocket?.sizeId != pocketSize.id) {
          ...
          Get.dialog(
            const NotifyClosePocketDialog(
              isDismissable: false,
              pocketCloseType: PocketCloseType.changePocketSize,
            ),
            barrierDismissible: false,
          );

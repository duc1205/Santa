...
import 'package:santapocket/modules/charity/app/ui/workflow/result/success/charity_pocket_send_complete_page.dart';
...
@injectable
class CharitySendPocketStatePageViewModel extends AppViewModel {
  final GetDeliveryUsecase _getDataUsecase;
  ...
  CharitySendPocketStatePageViewModel(
  ...
  Future<bool> _getData() async {
    ...
    final success = await run(
      () async {
        deliveryLoaded = await _getDataUsecase.run(deliveryId);
      },
    );
    await hideLoading();
    if (success) {
      ...
      gratefulMessage = deliveryLoaded.charityCampaign?.gratefulMessage ?? (deliveryLoaded.charity?.gratefulMessage ?? "");
    }
    return success;
  }
  ...
  Future<Unit> endProcess() async {
    if (!isDeliveryFinish) {
      ...
      await showLoading();
      final success = await run(
        () async => deliveryStatus = (await _getDataUsecase.run(deliveryId)).status,
      );
      await hideLoading();
      if (success) {
